...
| `name`       | 見出し                   |
| `html_url`   | リリースへのリンク             |
| `published_at` | 日付表示・並び順            |
| `created_at` | `published_at` が無い場合の代替 |
| `prerelease` | ラベル表示・`prereleases` フィルタ |
| `draft`      | 除外判定                  |
| `body`       | リリース本文                |

Draft リリースは常に除外する。並び順は `published_at`（無い場合は `created_at`）の降順とする。
...
フィルタは取得後に適用し、`limit` はフィルタ適用後の件数に対して効く。
...
6. `tagPattern` と `since` と `limit` がこの順で適用される
