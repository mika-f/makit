...
export async function DocsPage({ page, site, i18n, navigation, components: C }: DocsPageProps) {
  ...
  const columns = page.sidebar
    ? page.tableOfContents
      ? "md:grid-cols-[16rem_minmax(0,1fr)] xl:grid-cols-[16rem_minmax(0,1fr)_15rem]"
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[var(--makit-color-background)] text-[var(--makit-color-foreground)]">
      ...
      />
      <div
        className={`mx-auto grid w-full max-w-[88rem] flex-1 grid-cols-1 px-5 sm:px-8 ${columns}`}
      >
        {page.sidebar && (
          <C.Sidebar navigation={navigation} currentRoute={page.route} components={C} />
        )}
        <main data-pagefind-body className="min-w-0 py-12 md:px-10 md:py-16 xl:px-14">
