...
A `category.makit.ts` inside a [route group](../03-guides/02-content-structure.md#route-groups) directory still applies under the default `navigation.auto.routeGroups: "url"`. Under `"flatten"`, the group has no section of its own to attach to, so the file is ignored with a `route-group-category-ignored` warning.
